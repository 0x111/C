//Searching maximum in sector && adding on the sector via sector trees
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
showmax(int *max, int now, int tle, int tri, int le, int ri, int *sum)
{
    ...
    int tm = (tle + tri) / 2;
    push(sum, now, max);
    return maxi(showmax(max, now * 2, tle,
                        tm, le, mini(ri, tm), sum),
                showmax(max, now * 2 + 1, tm + 1,
                        tri, maxi(le, tm+1), ri, sum));
}

void //adding sum on the section
update(int *sum, int now, int tl, int tr,
    int l, int r, int add, int *max)
{
    if (l > r) {
        return;
    }
    if (l == tl && tr == r){
        ...
        int tm = (tl + tr) / 2;
        update (sum, now * 2, tl,
                tm, l, mini(r, tm), add, max);
        update (sum, now * 2 + 1, tm + 1,
                tr, maxi(l, tm + 1), r, add, max);
    }
    max[now]=maxi(max[now * 2]+sum[now * 2], max[now * 2 + 1]+sum[now * 2 + 1]);
    return;
}
